...
function unitConvSetup() {
  var unitDict = {
    ...
    Volume: {
      ...
      "Cubic Milimeter": 1.0e-9,
      Liter: 0.001,
      Mililiter: 0.000001,
      "US Gallon": 0.00378541,
      "US Quart": 0.0009463525,
      "US Pint": 0.00047317625,
      "US Cup": 0.000236588125,
      "US Fluid Ounce": 0.000029573515625,
      "US Table Spoon": 0.0000147867578125,
      "US Tea Spoon": 4.9289192708333333333333333333333e-6,
      "Imperial Gallon": 0.00454609,
      "Imperial Quart": 0.0011365225,
      "Imperial Pint": 0.00056826125,
      "Imperial Fluid Ounce": 0.0000284130625,
      "Imperial Table Spoon": 0.0000177581640625,
      "Imperial Tea Spoon": 5.9193880208333333333333333333333e-6,
    ...
    Weight: {
      ...
      Ounce: 0.0283495,
      Carrat: 0.0002,
      "Atomic Mass Unit": 1.6605401999104288e-27,
    },
  };
  ...
  form_c.find('input[name="convert"]').click(function () {
    doConversion();
  });
    ...
    if (isMobile) {
      ...
      for (var key in unitDict[optionClass]) {
        form_c
          .find(".optionList")
          .append(
            "<input type='button' readonly name='" +
              key +
              "' value='" +
              key +
              "'>"
  ...
  function doConversion() {
    ...
    if (isMobile) {
      ...
      var result =
        ...
        l.find("input[name='from-val']").val() +
        " " +
        unit_from +
        " = " +
        parseNum(val) +
...
function calculatorSetup() {
  var isNumber = 0,
    ...
    isMobile = 0,
    calWidget = "";
  (isExecuted = 0), (buffer = "0"), (memory = 0);
  (operator = ""), (value = 0), (display = ""), (val2 = 0);
  ...
  function changeLayout() {
    ...
    if (calWidget) {
      calWidget.off();
      calWidget.find("input[type='button']").off();
    }
    ...
    calWidget.keyup(function (e) {
      var key = e.which || e.keyCode;
      if (key > 95 && key < 106)
        calWidget.find("input[value='" + (key - 96) + "']").click();
        ...
        calWidget.find("input[name='calculate']").click();
    ...
    calWidget.find("input[type='button']").click(function () {
      ...
      else if (c.attr("name") == "mem_plus") {
        if (display.val())
          memory += parseFloat(display.val().replace(/,/g, ""));
      } else if (c.attr("name") == "mem_minus") {
        if (display.val())
          memory -= parseFloat(display.val().replace(/,/g, ""));
      } else if (c.attr("name") == "mem_recall") {
        buffer = numberWithCommas(memory);
        updateDisplay(buffer);
      } else if (c.attr("name") == "percentButton") {
        if (value == 0) {
          if (display.val())
            buffer = numberWithCommas(
              parseFloat(display.val().replace(/,/g, "")) / 100
            );
        } else {
          buffer = numberWithCommas(
            (value * parseFloat(display.val().replace(/,/g, ""))) / 100
          );
        }
        updateDisplay(buffer);
      } else if (c.attr("name") == "root2") {
        if (display.val())
          buffer = numberWithCommas(
            Math.sqrt(parseFloat(display.val().replace(/,/g, "")))
          );
        updateDisplay(buffer);
      } else if (c.attr("name") == "piConst") {
        buffer = Math.PI;
        updateDisplay(buffer);
      } else if (c.attr("name") == "negate") {
        if (display.val())
          buffer = numberWithCommas(
            parseFloat(display.val().replace(/,/g, "")) * -1
          );
        updateDisplay(buffer);
      } else if (c.attr("name") == "squareVal") {
        if (display.val())
          buffer =
            "" +
            numberWithCommas(
              Math.pow(parseFloat(display.val().replace(/,/g, "")), 2)
            );
        updateDisplay(buffer);
      } else if (c.attr("name") == "backspaceBtn") {
        backspaceEntry();
      } else if (c.attr("name") == "oneoverx") {
        if (display.val()) {
          buffer = numberWithCommas(
            1 / parseFloat(display.val().replace(/,/g, ""))
          );
          updateDisplay(buffer);
        }
      }
  ...
  function enterOperator(c) {
    if (isExecuted) {
      ...
      if (c == "\u003D") {
        ...
        if (operator == "\u00D7") value = value * val2;
        if (operator == "\u00F7")
          val2 == 0 ? (value = "NaN") : (value = value / val2);
        if (operator == "+") value = value + val2;
      } else {
        operator = c;
        isOperator = 1;
  ...
  function displayClear() {
    if (calWidget.find("input[name='clearButton']").val() == "CE") {
      buffer = "0";
...
function percentCalcSetup() {
  var form_p = jQuery("#percentCalc"),
    type = 0;
  loadForm();
  form_p.find("select").change(function () {
    loadForm();
  });
  ...
  function loadForm() {
    ...
    if (type == "2") {
      ...
      form_p
        .find("#x-val .input-group-prepend, #y-val .input-group-prepend")
        .hide();
      ...
      form_p
        ...
        .show();
      form_p
        .find("#x-val .input-group-prepend .input-group-text")
        .text("Before Wash");
      form_p
        .find("#y-val .input-group-prepend .input-group-text")
        .text("After Wash");
      form_p.find("#x-val .input-group-append").hide();
      form_p
        .find("#text-top")
        .html("<b>What is the fabric % increase/de&shy;crease from</b>");
  ...
  function type2() {
    ...
    form_p
      .find("#pc-answer")
      .html("<b style='color:red;'>Answer: </b>" + parseFloat(result));
  }
